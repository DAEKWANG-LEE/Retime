...
# 백터에서 행렬로 바뀜.
x = np.array([range(1, 101), range(101, 201)])#.reshape(-1,2)
y = np.array([range(1, 101), range(101, 201)])#.reshape(-1,2)
...
# model.add(Dense(5, input_shape = (2,100)))
# input_shape = (2,) 을 2로 바꿔주자.
model.add(Dense(5, input_shape = (2,)))
model.add(Dense(4))
model.add(Dense(8))
model.add(Dense(8))
model.add(Dense(4))
# model.add(Dense(1)) #y가 2개이기 때문에 output도 2로 한다.
model.add(Dense(2))
