...
x1, y1 = split_xy5(samsung, 25, 1)
x2, y2 = split_xy5(kospi200, 25, 1)
...
x1_train = np.reshape(x1_train_scaled,(x1_train.shape[0],25,5))
x1_test = np.reshape(x1_test_scaled,(x1_test.shape[0],25,5))

x2_train = np.reshape(x2_train_scaled,(x2_train.shape[0],25,5))
x2_test = np.reshape(x2_test_scaled,(x2_test.shape[0],25,5))
...
model.fit([x1_train, x2_train], y1_train, 
          ...
          verbose=1, 
          batch_size= 1, epochs=10, callbacks=[early_stopping])

loss, mse = model.evaluate([x1_test,x2_test], y1_test, batch_size=1)

print( " mse :  ", mse)

# 코스피200 지수와 삼성전자의 시가 고가저가는 나와있다.   61100,  61200 ,60000,
# xx1_test = 
# xx2_test =
