...
df_data = pd.read_csv('./samsung.csv' , encoding='euc-kr' )

#오름차순 이기때문에 인덱스 0 기준으로 내림차순으로 변경
df_data = df_data.sort_index(axis=0, ascending=False)

del df_data['일자']

df_data['시가'] = df_data['시가'].str.replace(pat=r'[,]', repl= r'', regex=True).astype('float')
df_data['고가'] = df_data['고가'].str.replace(pat=r'[,]', repl= r'', regex=True).astype('float')
df_data['저가'] = df_data['저가'].str.replace(pat=r'[,]', repl= r'', regex=True).astype('float')
df_data['종가'] = df_data['종가'].str.replace(pat=r'[,]', repl= r'', regex=True).astype('float')
df_data['거래량']= df_data['거래량'].str.replace(pat=r'[,]', repl= r'', regex=True).astype('float')

df_data.rename(columns={'시가':'c1','고가':'c2','저가':'c3','종가':'c4','거래량':'c5'}, inplace=True)
...
for a in range(1,421):
    x_data = np.reshape(x1[a:a+5,:],(1,5,5))
