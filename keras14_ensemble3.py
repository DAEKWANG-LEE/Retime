...
x1_train, x1_test, y1_train, y1_test  = train_test_split(
    x1, y1, train_size = 0.6, random_state = 66, # random_state 66번째 의 랜덤스테이트 모듈을 사용하겠다(랜덤 난수 사용).
    shuffle = False
)

x1_val, x1_test,  y1_val, y1_test = train_test_split(
    x1_test, y1_test, test_size = 0.5, random_state = 66, # random_state 66번째 의 랜덤스테이트 모듈을 사용하겠다(랜덤 난수 사용).
    shuffle = False
)
...
#분개 시키자.
#레이어를 정의 해주자.
output_1 = Dense(4)(output1) # 1번째 아웃풋 모델 
output_1 = Dense(8)(output1) 
output_1 = Dense(16)(output1)
output_1 = Dense(16)(output1)
output_1 = Dense(8)(output1) 
...
# 4. 평가 예측
aaa = model.evaluate(x1_test, [y1_test, y2_test, y3_test], batch_size = 2 )
...
print('========================')
'''
aaa: [총 loss, loss 3개 mse 3개 나옴. 총 그래서 7개 값이 나옴.
3.177629992023867e-06, < total loss
2.4141627e-08,  < 첫번째 loss
2.9134253e-06,  < 두번째 loss
2.4006295e-07,  < 세번째 loss
0.0001344045,  < 첫번째 mse
0.0014953614,  < 두번째 mse
0.0004426321 ] < 세번째 mse
'''
...
predict_aa = model.predict(x1_prd, batch_size = 2 )
# print("predict_aa : ", predict_aa)

#  y_predict 만들어 주자.  / list 를 추가해주자.
#  y1_predict[0] model.predict 
y1_predict = model.predict(x1_test, batch_size = 2 )
...
print("RMSE1 + RMSE2 + RMSE3: ", rm1, rm2, rm3)
...
print("pre1 + pre2 + pre 3: ", r2_y_predict1, r2_y_predict2, r2_y_predict3)
